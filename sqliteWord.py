...
class sqliteWord:
    ...
    def searchWord(self, text):
        ...
        #     self.connect()
        cur = self.cursor()
        cur.execute('SELECT _id FROM words WHERE name=?', (text,))
        try:
            res = cur.fetchone()[0]
        except:
            res = 0
    ...
    def appendLinkIdTextWord(self, idtext, word, typeword):
        idword = self.appendWord(word, typeword)
        arr = (idtext, idword, )
        cur = self.cursor()
        cur.execute(
            'select * from textwordlinks WHERE id_text=? and id_word=? ', arr)
    ...
    def searchWordComment(self, id_word):
        # if self.conn is None:
        #     self.connect()

        cur = self.cursor()
        cur.execute('SELECT _id FROM wordcomments WHERE id_word=?', (id_word,))
        try:
            res = cur.fetchone()[0]
        except:
            res = 0
    ...
    def appendWordComment(self, word, comment):
        id_word = self.searchWord(word)
        res = self.searchWordComment(id_word)
        if not res:
            cur = self.cursor()
            cur.execute(
                'INSERT INTO wordcomments(id_word,id_type,name) VALUES (?,?,?)', (id_word, 1, comment))
            res = cur.lastrowid
            self.commit()
        return res

    def writeComments(self, idtext):
        with open('ankiCard{}.txt'.format(idtext), 'w', encoding="utf-8") as fh:
            cur = self.cursor()
            for row in cur.execute('''            
                    SELECT a.word,a.ukr,a.comment
                            ...
                            order by l.counts desc
                    ''', (idtext,)):
                txt = ''.join(field+'\t' for field in row)+'\n'
                print(txt)
                fh.write(txt)
